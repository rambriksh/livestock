...
@Component({
  selector: 'app-transportregistration',
  templateUrl: './transportregistration.component.html',
  styleUrls: ['./transportregistration.component.css']
})
export class TransportregistrationComponent implements OnInit {
  ...
  genders=[
    ...
    var id = sData.id;
     this.stateName = sData.name;
      ...
      this.httpClient.get('http://localhost:8111/city1/books').subscribe((res2)=>{
          debugger;
  ...
  ngOnInit() {
     // this.countryLoad();
     this.httpClient.get('http://localhost:8111/country1/books').subscribe((res)=>{
  ...
  onSubmit() {
    ...
    this.httpClient.post("http://localhost:8111/transport/books",
