...
@Component({
  selector: 'app-certificationregistration',
  templateUrl: './certificationregistration.component.html',
  styleUrls: ['./certificationregistration.component.css']
})
export class CertificationregistrationComponent implements OnInit {
  submitted = false;
  user: User = new User();
  ...
  genders=[
    ...
    getCountry(cData){
        ...
        this.httpClient.get('http://localhost:8111/state1/books').subscribe((res1)=>{
    ...
    var id = sData.id;
     this.stateName = sData.name;
      ...
      this.httpClient.get('http://13.233.94.207:8111/city1/books').subscribe((res2)=>{
          debugger;
  ...
  ngOnInit() {
     // this.countryLoad();
     this.httpClient.get('http://13.233.94.207:8111/country1/books').subscribe((res)=>{
  ...
  onSubmit() {
    ...
    this.httpClient.post("http://13.233.94.207:8111/certification/books",
